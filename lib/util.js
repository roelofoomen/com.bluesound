...
class Util {
  ...
  getBluesound(address, port, useLongPolling, pollingInterval, etag) {
    return new Promise((resolve, reject) => {
      if (useLongPolling) {
        var fetchCommand = 'http://'+ address +':'+ port +'/Status?timeout=' + pollingInterval + '&etag=' + etag;
        var fetchTimeout = (pollingInterval + 2) * 1000;
      } else {
        var fetchCommand = 'http://'+ address +':'+ port +'/Status';
        var fetchTimeout = (pollingInterval - 2) * 1000;
      }
