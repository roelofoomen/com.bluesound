...
class Util {
  ...
  sendCommand(command, address, port, state = '') {
    return new Promise((resolve, reject) => {
      // State value for repeat and shuffle
      if (state.toString() != '') {
        state = "?state=" + state
      }
      fetch('http://'+ address +':'+ port +'/'+ command + state, {
          method: 'GET',
          timeout: 4000
        })
        ...
        .then(body => {
          ...
          parser.parseStringPromise(body)
            .then(result => {
              ...
              } else if (result.status.hasOwnProperty("service")) {
                if (title2 !== artist && result.status.service[0] !== 'LocalMusic') {
                  var track = title2;
                }
